...
public class BanWaveCommands {
    ...
    public void banWave(CommandArgs args) {
        ...
        ChatUtil.sendCenteredMessage(pl, "&cYou are using BanWaves 1.0-Stable!");
        ChatUtil.sendCenteredMessage(pl, "&cDeveloped By &a&nEos!");
    }
    ...
    public void banWaveOn(CommandArgs args) {
        ...
        if ( BanWaves.getInstance().getConfig().getBoolean("BanWaves.EnableBanWave") ) {
            PlayerUtil.sendMessage(pl, "&c[BanWaves] &c&n&lBAN WAVE IS ALREADY ENABLED!");
        } else {
    ...
    public void banWaveOff(CommandArgs args) {
        Player pl = args.getPlayer();
        if ( !BanWaves.getInstance().getConfig().getBoolean("BanWaves.EnableBanWave") ) {
            PlayerUtil.sendMessage(pl, "&c[BanWaves] &c&n&lBAN WAVE IS ALREADY DISABLED!");
        } else {
    ...
    public void banWaveExecute(CommandArgs args) {
        ...
        if ( BanWaveUtil.getAmountToBan() == 0 ) {
            PlayerUtil.sendMessage(pl, "&c[BanWaves] &c&n&lNO USERS TO BAN!");
            return;
        }
    ...
    public void banWaveReload(CommandArgs args) {
        Player pl = args.getPlayer();
        PlayerUtil.sendMessage(pl, "&aReloaded Banwaves config | &eDeveloper Eos @ spigot.org");
